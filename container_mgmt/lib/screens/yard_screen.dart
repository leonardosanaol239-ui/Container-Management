...
class YardScreen extends StatefulWidget {
  ...
  const YardScreen({super.key, required this.yard, required this.portId, required this.portName});
  @override
  State<YardScreen> createState() => _YardScreenState();
}

class _YardScreenState extends State<YardScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _blinkCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 500));
    _loadAll();
  }
  ...
  Future<void> _searchContainer() async {
    ...
    setState(() { _foundContainer = c; _highlightedRowId = c?.rowId; }); if (c?.rowId != null) { _blinkCtrl.repeat(reverse: true); }
  }

  void _clearSearch() { setState(() { _searchCtrl.clear(); _foundContainer = null; _highlightedRowId = null; }); _blinkCtrl.stop(); _blinkCtrl.reset(); }
  ...
  Future<void> _dropToSlot(ContainerModel container, int rowId) async {
    ...
    for (final rows in _rowsByBay.values) {
      for (final r in rows) { if (r.rowId == rowId) { targetRow = r; break; } }
      if (targetRow != null) break;
    }
    if (existing.length >= (targetRow?.maxStack ?? 5)) return;
    int? blockId, bayId;
    for (final e in _rowsByBay.entries) {
      for (final r in e.value) { if (r.rowId == rowId) { bayId = e.key; break; } }
      if (bayId != null) break;
    }
    for (final e in _baysByBlock.entries) {
      for (final b in e.value) { if (b.bayId == bayId) { blockId = e.key; break; } }
      if (blockId != null) break;
    }
    if (blockId == null || bayId == null) return;
    try {
      await _api.moveContainer(containerId: container.containerId, yardId: widget.yard.yardId, blockId: blockId, bayId: bayId, rowId: rowId, tier: existing.length + 1);
      await _loadAll();
    } catch (_) {}
  }

  Future<void> _returnToHolding(ContainerModel container) async {
    try { await _api.removeContainerFromSlot(container.containerId); await _loadAll(); } catch (_) {}
  }

  void _toggleEditMode() {
    setState(() {
      _editMode = !_editMode;
      if (!_editMode) { _selectedSlotId = null; _selectedSlotEdit = null; _selectedOrientationId = null; _selectedSizeId = null; }
    });
  }
  ...
  Future<void> _saveLayout() async {
    ...
    setState(() { _saving = false; _editMode = false; _selectedSlotId = null; });
  }

  void _showAddBlockDialog() {
    showDialog(context: context, builder: (_) => _AddBlockDialog(
      sizes: _sizes, orientations: _orientations,
      onConfirm: (name, bays, rows, orientId, sizeId, maxStack) async {
        await _api.createBlock(yardId: widget.yard.yardId, portId: widget.portId, blockName: name, numBays: bays, numRows: rows, orientationId: orientId, sizeId: sizeId, maxStack: maxStack, posX: 10, posY: 10);
        await _loadAll();
      },
    ));
  }
  ...
  Future<void> _applyOrientationChange(int rowId, int orientationId) async {
    await _api.updateRow(rowId, orientationId: orientationId); await _loadAll();
  }

  Future<void> _applySizeChange(int rowId, int sizeId) async {
    await _api.updateRow(rowId, sizeId: sizeId); await _loadAll();
  }
  ...
  Future<void> _applySlotEdit(String action) async {
    ...
    if (action == 'stackMax') {
      ...
      try {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cannot delete: row has containers')));
      }
    }
  }
  ...
  void _showStackMaxDialog(int rowId) {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: TextField(controller: ctrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Max Stack (e.g. 5)', border: OutlineInputBorder())),
      actions: [
  ...
  Future<void> _showMoveOutDialog(ContainerModel container) async {
    ...
    if (!mounted) return;
    await showDialog(context: context, builder: (_) => _MoveOutDialog(
      container: container, trucks: trucks,
      onConfirm: (truckId, boundTo) async {
        await _api.moveOutContainer(containerId: container.containerId, truckId: truckId, boundTo: boundTo);
        await _loadAll();
      },
    ));
  }
  ...
  void _closeTierPopup() => setState(() { _tierPopupRowId = null; _tierPopupPosition = null; });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('${widget.portName}  >  Yard ${widget.yard.yardNumber}',
            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
      ),
      body: _loading
          ? const Center(child: CircularProgressIndicator())
          : Stack(key: _yardKey, children: [
              Padding(
                  ...
                  DragTarget<ContainerModel>(
                    ...
                    builder: (ctx, candidates, _) => Container(
                      decoration: candidates.isNotEmpty
                          ? BoxDecoration(border: Border.all(color: Colors.blue, width: 4), borderRadius: BorderRadius.circular(12))
                          : null,
                      child: ContainerHoldingArea(portId: widget.portId, containers: _containers, onRefresh: _loadAll),
                    ),
                  ),
              ...
              if (_tierPopupRowId != null && _tierPopupPosition != null)
                Positioned(
                  left: _tierPopupPosition!.dx, top: _tierPopupPosition!.dy,
                  child: _TierPopup(containers: _containersByRow[_tierPopupRowId!] ?? [], onClose: _closeTierPopup),
                ),
              if (_showMoveOutList && _movedOutContainers.isNotEmpty)
                Positioned(left: 220, top: 56,
                  child: Material(elevation: 8, borderRadius: BorderRadius.circular(8),
                    child: Container(
                      width: 240, constraints: const BoxConstraints(maxHeight: 300),
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          return ListTile(dense: true,
                            title: Text(c.containerNumber, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                            subtitle: Text(c.boundTo != null ? 'Bound to: ${c.boundTo}' : '', style: const TextStyle(fontSize: 11)));
                        },
                      ),
                    ),
                  )),
  ...
  Widget _buildToolbar() {
    return Row(children: [
      ElevatedButton.icon(
        ...
        label: Text(_editMode ? (_saving ? 'Saving...' : 'Save Layout') : 'Edit Layout'),
      ...
      if (_editMode) ...[
        ...
        TextButton(onPressed: _toggleEditMode, child: const Text('Cancel', style: TextStyle(color: Colors.red))),
      ],
        ...
        builder: (ctx, candidates, _) => GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: candidates.isNotEmpty ? Colors.red[700] : Colors.red[600],
              borderRadius: BorderRadius.circular(8),
            ),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.local_shipping, color: Colors.white, size: 18),
              const SizedBox(width: 6),
              Text('Move Out${_movedOutContainers.isNotEmpty ? " (${_movedOutContainers.length})" : ""}',
      ...
      Expanded(child: Container(
        ...
        child: Row(children: [
          Expanded(child: TextField(
            ...
            decoration: const InputDecoration(hintText: 'Search container number', border: InputBorder.none, contentPadding: EdgeInsets.symmetric(horizontal: 12), isDense: true),
            onSubmitted: (_) => _searchContainer(),
          )),
          if (_searchCtrl.text.isNotEmpty) IconButton(icon: const Icon(Icons.close, size: 18), onPressed: _clearSearch),
          ElevatedButton(
            onPressed: _searchContainer,
            style: ElevatedButton.styleFrom(backgroundColor: Colors.amber, foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6))),
            child: const Text('Locate', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
          ),
        ]),
      )),
    ]);
  ...
  Widget _buildEditToolbar() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.amber[50], border: Border.all(color: Colors.amber), borderRadius: BorderRadius.circular(8)),
      child: Row(children: [
        ElevatedButton.icon(
          onPressed: _showAddBlockDialog,
          icon: const Icon(Icons.add, size: 16), label: const Text('Add Block'),
          style: ElevatedButton.styleFrom(backgroundColor: Colors.blue, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)), padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8)),
        ),
        const SizedBox(width: 10),
        _ToolbarDropdown<int>(
          label: 'Orientation', value: _selectedOrientationId, hint: 'Select',
          items: _orientations.map((o) => DropdownMenuItem(value: o.orientationId, child: Text(o.orientationDesc, style: const TextStyle(fontSize: 12)))).toList(),
          onChanged: (v) { setState(() => _selectedOrientationId = v); if (v != null && _selectedSlotId != null) _applyOrientationChange(_selectedSlotId!, v); },
        ),
        const SizedBox(width: 10),
        _ToolbarDropdown<int>(
          label: 'Cell Size', value: _selectedSizeId, hint: 'Select',
          items: _sizes.map((s) => DropdownMenuItem(value: s.sizeId, child: Text(s.sizeDesc, style: const TextStyle(fontSize: 12)))).toList(),
          onChanged: (v) { setState(() => _selectedSizeId = v); if (v != null && _selectedSlotId != null) _applySizeChange(_selectedSlotId!, v); },
        ),
        const SizedBox(width: 10),
        _ToolbarDropdown<String>(
          label: 'Slot Edit', value: _selectedSlotEdit, hint: 'Select',
          items: const [
            DropdownMenuItem(value: 'stackMax', child: Text('Stack Max', style: TextStyle(fontSize: 12))),
            DropdownMenuItem(value: 'deleteRow', child: Text('Delete Row', style: TextStyle(fontSize: 12))),
          ],
          onChanged: (v) { setState(() => _selectedSlotEdit = v); if (v != null) _applySlotEdit(v); },
        ),
        const Spacer(),
        if (_selectedSlotId != null) Text('Row #$_selectedSlotId selected', style: const TextStyle(fontSize: 11, color: Colors.grey)),
        const SizedBox(width: 10),
        OutlinedButton.icon(
          onPressed: _selectedSlotId != null ? () async {
            try {
              await _api.deleteRow(_selectedSlotId!);
              setState(() => _selectedSlotId = null);
              await _loadAll();
            } catch (_) {
              if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cannot delete: slot has containers')));
            }
          } : null,
          icon: const Icon(Icons.delete, size: 16, color: Colors.red),
          label: const Text('Delete Slot', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
          style: OutlinedButton.styleFrom(side: const BorderSide(color: Colors.red), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)), padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8)),
        ),
      ]),
    );
  }
  ...
  Widget _buildPositionedBlock(Block block) {
    ...
    final offsetFt = _blockOffsets[block.blockId] ?? Offset((block.posX ?? 10).toDouble(), (block.posY ?? 10).toDouble());
    final offset = Offset(offsetFt.dx * _scale, offsetFt.dy * _scale);

    final bw = _BlockWidget(

      block: block, baysByBlock: _baysByBlock, rowsByBay: _rowsByBay, containersByRow: _containersByRow,

      highlightedRowId: _highlightedRowId, blinkCtrl: _blinkCtrl, editMode: _editMode, selectedRowId: _selectedSlotId,

      scaleX: _scaleX, scaleY: _scaleY,

      onSlotTap: _editMode ? null : (rowId, pos) => _showSlotTierPopup(rowId, pos),

      onSlotDrop: _editMode ? null : _dropToSlot,

      onSelectRow: _editMode ? (rowId) => setState(() => _selectedSlotId = rowId) : null,

      onAddBay: _editMode ? () async { await _api.addBay(block.blockId); await _loadAll(); } : null,

      onRemoveBay: _editMode ? () async {

        try { await _api.removeBay(block.blockId); await _loadAll(); }

        catch (_) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cannot remove bay'))); }

      } : null,

      onDeleteBlock: _editMode ? () async {

        try { await _api.deleteBlock(block.blockId); setState(() => _selectedSlotId = null); await _loadAll(); }

        catch (_) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cannot delete: block has containers'))); }

      } : null,

      onAddRow: _editMode ? () async { try { await _api.addRow(block.blockId); await _loadAll(); } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Cannot add row: ' + e.toString()), duration: const Duration(seconds: 4))); } } : null,

      onRemoveRow: _editMode ? () async { try { await _api.removeRow(block.blockId); await _loadAll(); } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Cannot remove row: ' + e.toString()), duration: const Duration(seconds: 4))); } } : null,

    );

    if (!_editMode) return Positioned(left: offset.dx, top: offset.dy, child: bw);
      ...
      child: GestureDetector(
        ...
        onPanUpdate: (d) {
          ...
          setState(() {
            ...
            final proposed = Offset(cur.dx + d.delta.dx / _scale, cur.dy + d.delta.dy / _scale);
        ...
        onPanEnd: (_) async {
          ...
          for (final other in _blocks) {
            ...
            final otherPos = _blockOffsets[other.blockId] ?? Offset((other.posX ?? 10).toDouble(), (other.posY ?? 10).toDouble());
            final otherRect = _getSlotsRect(other, otherPos);
            if (myRect.overlaps(otherRect)) { overlaps = true; break; }
          }
          if (overlaps) {
            setState(() => _blockOffsets[block.blockId] = Offset((block.posX ?? 10).toDouble(), (block.posY ?? 10).toDouble()));
            if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Slot areas cannot overlap'), duration: Duration(seconds: 1)));
            return;
          }
          try { await _api.updateBlockPosition(block.blockId, myPos.dx, myPos.dy); } catch (_) {}
...
class _BlockWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final borderColor = isVert ? Colors.teal.shade700 : Colors.blueGrey.shade700;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onTapUp: editMode
          ...
          : (d) { onTap?.call(d.globalPosition); },

      child: DragTarget<ContainerModel>(

        onWillAcceptWithDetails: (d) => !editMode && !d.data.isMovedOut && inYard.length < maxT,

        onAcceptWithDetails: (d) { onDrop?.call(d.data); },
        ...
        builder: (ctx, candidates, _) {
          ...
          final bgColor = topContainer == null
              ...
              : (topContainer.statusId == 1 ? Colors.amber.shade300 : Colors.red.shade300);
                      ...
                      if (inYard.length > 1)
                        ...
                        Positioned(
                          ...
                          child: Text(
                            ...
                            style: TextStyle(fontSize: 6, color: Colors.grey.shade700, fontWeight: FontWeight.bold),
              ...
              feedback: Material(
                ...
                child: Container(
                  ...
                  child: Center(
                    ...
                    child: Text(topContainer.containerNumber,
...
class _AddBlockDialog extends StatefulWidget {
  ...
  final Future<void> Function(String name, int bays, int rows, int orientId, int sizeId, int maxStack) onConfirm;
  const _AddBlockDialog({required this.sizes, required this.orientations, required this.onConfirm});
  @override
...
class _AddBlockDialogState extends State<_AddBlockDialog> {
  ...
  @override
  void dispose() { _nameCtrl.dispose(); _baysCtrl.dispose(); _rowsCtrl.dispose(); _stackCtrl.dispose(); super.dispose(); }
      ...
      child: Container(width: 360, padding: const EdgeInsets.all(20),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Add Block', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          ...
          const SizedBox(height: 8),
          const Text('Block Orientation:', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
          const SizedBox(height: 4),
          DropdownButton<int>(value: _orientId, isExpanded: true, hint: const Text('Select orientation'),
            items: widget.orientations.map((o) => DropdownMenuItem(value: o.orientationId, child: Text(o.orientationDesc))).toList(),
            onChanged: (v) => setState(() => _orientId = v)),
          const SizedBox(height: 8),
          const Text('Container Size:', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
          const SizedBox(height: 4),
          DropdownButton<int>(value: _sizeId, isExpanded: true, hint: const Text('Select size'),
            items: widget.sizes.map((s) => DropdownMenuItem(value: s.sizeId, child: Text(s.sizeDesc))).toList(),
            onChanged: (v) => setState(() => _sizeId = v)),
          const SizedBox(height: 16),
          SizedBox(width: double.infinity, child: ElevatedButton(
            onPressed: _loading ? null : _submit,
            style: ElevatedButton.styleFrom(backgroundColor: Colors.blue, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), padding: const EdgeInsets.symmetric(vertical: 12)),
            child: _loading ? const SizedBox(height: 18, width: 18, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Text('Create Block', style: TextStyle(fontWeight: FontWeight.bold)),
          )),
        ]),
      ),
    );
  ...
  Widget _field(String label, TextEditingController ctrl, {String? hint, bool numeric = false}) {
    return Padding(padding: const EdgeInsets.only(bottom: 10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 4),
      TextField(controller: ctrl, keyboardType: numeric ? TextInputType.number : TextInputType.text,
        decoration: InputDecoration(hintText: hint, border: const OutlineInputBorder(), contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), isDense: true)),
    ]));
  }
  ...
  Future<void> _submit() async {
    ...
    if (name.isEmpty || bays < 1 || rows < 1 || _orientId == null || _sizeId == null) return;
    setState(() => _loading = true);
...
class _TierPopupState extends State<_TierPopup> {
  ...
  @override
  Widget build(BuildContext context) {
    return Material(elevation: 10, borderRadius: BorderRadius.circular(12),
      child: Container(width: 230, padding: const EdgeInsets.all(14),
  ...
  Widget _buildList() {
    return Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        const Text('Tier', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
        GestureDetector(onTap: widget.onClose, child: const Icon(Icons.close, color: Colors.red, size: 18)),
      ]),
      const SizedBox(height: 8),
      ...widget.containers.map((c) => GestureDetector(
        ...
        child: Container(
          ...
          child: Row(children: [
            Text('${c.tier}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            const SizedBox(width: 10),
            Container(width: 16, height: 16, decoration: BoxDecoration(color: c.statusId == 1 ? Colors.amber : Colors.red, borderRadius: BorderRadius.circular(3))),
            const SizedBox(width: 8),
            Expanded(child: Text(c.containerNumber, style: const TextStyle(color: Colors.white, fontSize: 13), overflow: TextOverflow.ellipsis)),
          ]),
        ),
      )),
    ]);
  ...
  Widget _buildDetails(ContainerModel c) {
    return Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        GestureDetector(onTap: () => setState(() => _selected = null), child: const Icon(Icons.arrow_back, color: Colors.white70, size: 18)),
        GestureDetector(onTap: widget.onClose, child: const Icon(Icons.close, color: Colors.red, size: 18)),
      ]),
      const SizedBox(height: 8),
      Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 8),
        decoration: BoxDecoration(color: Colors.amber, borderRadius: BorderRadius.circular(6)),
        child: Text(c.containerNumber, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.black))),
      ...
      Container(width: double.infinity, padding: const EdgeInsets.all(8),
        ...
        child: Text(c.containerDesc ?? '-', style: const TextStyle(color: Colors.white, fontSize: 12))),
    ]);
  }
...
class _MoveOutDialog extends StatefulWidget {
  ...
  const _MoveOutDialog({required this.container, required this.trucks, required this.onConfirm});
  @override
...
class _MoveOutDialogState extends State<_MoveOutDialog> {
  ...
  @override
  void dispose() { _boundCtrl.dispose(); super.dispose(); }

  @override
  Widget build(BuildContext context) {
    return Dialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      child: Container(width: 340, padding: const EdgeInsets.all(20),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Move Out Container', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            IconButton(icon: const Icon(Icons.close, size: 20), onPressed: () => Navigator.pop(context), padding: EdgeInsets.zero, constraints: const BoxConstraints()),
          ]),
          const Divider(),
          Text(widget.container.containerNumber, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          const SizedBox(height: 12),
          const Text('Truck:', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
          const SizedBox(height: 4),
          DropdownButton<int>(value: _truckId, isExpanded: true, hint: const Text('Select truck'),
            items: widget.trucks.map((t) => DropdownMenuItem(value: t.truckId, child: Text(t.truckName))).toList(),
            onChanged: (v) => setState(() => _truckId = v)),
          const SizedBox(height: 10),
          const Text('Bound To:', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
          const SizedBox(height: 4),
          TextField(controller: _boundCtrl, decoration: const InputDecoration(hintText: 'Destination / consignee', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 10, vertical: 8), isDense: true)),
          const SizedBox(height: 16),
          SizedBox(width: double.infinity, child: ElevatedButton(
            onPressed: _loading || _truckId == null ? null : _submit,
            style: ElevatedButton.styleFrom(backgroundColor: Colors.red[600], foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), padding: const EdgeInsets.symmetric(vertical: 12)),
            child: _loading ? const SizedBox(height: 18, width: 18, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Text('Confirm Move Out', style: TextStyle(fontWeight: FontWeight.bold)),
          )),
        ]),
      ),
    );
...
class _SearchResultCard extends StatelessWidget {
  ...
  final Map<int, List<RowModel>> rowsByBay;
  final VoidCallback onClose;
  const _SearchResultCard({required this.container, required this.blocks, required this.baysByBlock, required this.rowsByBay, required this.onClose});
  ...
  @override
  Widget build(BuildContext context) {
    return Material(elevation: 8, borderRadius: BorderRadius.circular(10),
      child: Container(width: 240, padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.amber, width: 1.5)),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Found', style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 13)),
            GestureDetector(onTap: onClose, child: const Icon(Icons.close, color: Colors.white54, size: 16)),
          ]),
          const SizedBox(height: 6),
          Text(container.containerNumber, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)),
          const SizedBox(height: 4),
          Text(_locationLabel(), style: const TextStyle(color: Colors.white70, fontSize: 11)),
        ]),
      ),
    );
  }
